...
describe Calendar do
  before :each do
    @user1 = User.create!({:user_type => 1, :name => 'anoriginalname', :approved => true, :initials => "ZZ"})
    @user2 = User.create!({:user_type => 1, :name => 'mockuser', :approved => true, :initials => ":)"})
  ...
  describe "check if entry can be added to calendar" do
    ...
    it "should allow back to back entries at the same location" do
      @calendar1.canAdd(@backtobacksame).should == false
    end
  end
end
