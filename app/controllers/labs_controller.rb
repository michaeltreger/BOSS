class LabsController < ApplicationController
  ...
  def commit_shifts
    if @lab= Lab.find(params[:id])
      ...
      startWeek = Time.new(Time.current.year, timeTable[0][2]["start_time_month"], timeTable[0][3]["start_time_day"], 0, 0, 0, Time.current.to_s[-5..-3] + ":00")
      endWeek =  Time.new(Time.current.year, timeTable[0][4]["end_time_month"], timeTable[0][5]["end_time_day"].to_i + 1, 0, 0, 0, Time.current.to_s[-5..-3] + ":00")
                ...
                if timeTable[i][j] != {}
                  ...
                  timeTable[i][j].each do |k, v|
                    ...
                    if k =='XX' or k == 'xx'
                      ...
                      Substitution.create!(:entry => xxEntry, :entry_id => xxEntry.id, :description => 'This is an open shift')
                    else
                      ...
                      if v == 0
                        ...
                        realEndTime = startTime + 30.minute
                        employee.shift_calendar.entries << Entry.create!(:entry_type => 'shift', :user_id => employee.id, :start_time => startTime, :end_time => realEndTime, :description => "#{employee.name}@#{@lab.name}", :lab_id => @lab.id)
                      elsif v == 2
                        realStartTime = startTime + 30.minute
                        employee.shift_calendar.entries << Entry.create!(:entry_type => 'shift', :user_id => employee.id, :start_time => realStartTime, :end_time => endTime, :description => "#{employee.name}@#{@lab.name}", :lab_id => @lab.id)
                      end
                    end
                  end
                end
